...
struct mzROIStruct * cleanup(const int ctScan, struct mzROIStruct *mzROI, struct mzROIStruct *mzval, int imaxskip, struct mzLengthStruct *mzLength, int *scerr, struct pickOptionsStruct *pickOptions){
int i,p,del=0;
 ...
 for (i=0; i < mzLength->mzval; i++) {
	...
	if ((lastscan + imaxskip) < ctScan) {
		if ((entries >= pickOptions->minEntries) && (mzval[i].kI >= pickOptions->minimumIntValues)) {
...
SEXP findmzROI(SEXP mz, SEXP intensity, SEXP scanindex, SEXP mzrange, SEXP scanrange, SEXP lastscan, SEXP dev, SEXP minEntries, SEXP prefilter, SEXP noise, SEXP maxskip) {
  ...
  int i,*pscanindex, scanrangeFrom, scanrangeTo, ctScan, nmz, lastScan, inoise, imaxskip;
  ...
  for (ctScan=scanrangeFrom;ctScan<=scanrangeTo;ctScan++)
  {
    ...
    mzROI=cleanup(ctScan,mzROI,mzval,imaxskip,&mzLength,&scerr,&pickOptions);
    R_FlushConsole();
  } //for ctScan

  mzROI=cleanup(ctScan+1,mzROI,mzval,imaxskip,&mzLength,&scerr,&pickOptions);
